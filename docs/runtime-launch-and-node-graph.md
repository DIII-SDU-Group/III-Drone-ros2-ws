...
5. The CLI requests the tmux view model from `iii_drone_supervision/tmux_spec.py` and creates the operator session.
6. `iii system start` performs lifecycle-style configure/activate control over managed nodes.

Important distinction:
- ROS 2 launch is now the canonical source of process existence.
...
That path launches the same canonical graph but does not provide daemon-managed status, socket control, or tmux automation.

## 2. Launch Group Composition

The authoritative launch topology now comes from the canonical system specification in:
...
Examples from the current specification:
...
### 2.3 Real / OptiTrack profile entities
...
At the process level, the new canonical path is launch-driven:

- launch defines what exists
- the daemon tracks which processes are alive
...
3. CLI creates a tmux session whose panes observe logs and status.
4. Supervisor/configuration establish lifecycle control plane.
5. TF and sensor ingress are brought to active state via managed nodes.
6. Perception chain stabilizes powerline state.
7. Control primitives become available via action servers.
8. Mission executor registers PX4 modes and drives behavior trees.
9. Ground control observes status and injects operator commands/parameter changes.
