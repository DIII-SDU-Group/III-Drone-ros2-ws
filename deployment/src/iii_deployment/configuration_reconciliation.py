...
class ReceiverConfigurationReconciler:
    ...
    def __init__(
        ...
        *,
        ...
        if runtime_profile not in {"real", "opti_track", "hil"}:
            raise ContractError(
    ...
    @staticmethod
    def _configuration_root(container: Path) -> Path:
        """Resolve the package configuration root inside a durable container.

        Runtime checkpoints preserve the normal ``CONFIG_BASE_DIR/iii_drone``
        layout, while checkpoints produced before that boundary was formalised
        contain the package files directly at their root.  Accept both shapes,
        but reject an ambiguous container instead of selecting one implicitly.
        """

        nested = container / "iii_drone"
        markers = ("profiles", "parameter_sets", "state", "contracts")
        nested_present = nested.is_dir() and not nested.is_symlink()
        direct_present = any((container / marker).exists() for marker in markers)
        if nested_present and direct_present:
            raise ContractError("configuration container has ambiguous package roots")
        return nested if nested_present else container
    ...
    def _inputs(
        ...
        source_configuration = self._configuration_root(source)
        retained = source_configuration / "contracts" / old_manifest_id
        old_root = (
            retained
            if retained.is_dir() and not retained.is_symlink()
            else self._contract_root(old_release_id)
        )
        state_container = source
        ...
        if self.active_state_root is not None and self.active_state_root.exists():
            ...
            state = self._configuration_root(active)
        return (
            source,
            state_container,
            state,
            old_root,
            self._contract_root(release_id),
            source_manifest,
        )
    ...
    def preflight(
        ...
        *,
        ...
        api = self._api()
        try:
            source, state_container, state, old_root, new_root, source_manifest = self._inputs(
        ...
        predicted = api.plan_reconciled_checkpoint(
            ...
            source_state_root=state_container,
    ...
    def apply(
        ...
        *,
        ...
        staged_configuration = self._configuration_root(staged_state)
        if staged_configuration != staged_state:
            shutil.copy2(
                staged_state / ".iii-reconciliation-stage.json",
                staged_configuration / ".iii-reconciliation-stage.json",
            )
        try:
            plan = api.plan_reconciliation(
                old_immutable_root=old_root,
                new_immutable_root=new_root,
                writable_state_root=staged_configuration,
                operations_root=self.operations_root,
                operation_id=operation_id,
                runtime_profile=self.runtime_profile,
                target_id=self.target_id,
            ...
            if staged_configuration != staged_state:
                (staged_configuration / ".iii-reconciliation-stage.json").unlink()
