...
def _atomic_document(
    ...
    *,
    ...
    if temporary.exists() or temporary.is_symlink():
        raise ContractError(
            f"stale atomic state path requires reconciliation: {temporary.name}"
        )
...
class ReleaseStore:
    ...
    def __init__(
        ...
        *,
        ...
        if self.target_root == Path("/") and os.geteuid() != 0:
            raise ContractError(
                "onboard release staging requires receiver root authority"
            )
        ...
        try:
            ...
            raise ContractError(
                f"runtime group is not provisioned: {runtime_group}"
            ) from exc
        self.state_owner = (
            (0, self.runtime_gid) if self.target_root == Path("/") else None
        )
    ...
    def _validate_state(self, state: Mapping[str, Any]) -> dict[str, Any]:
        ...
        missing = sorted(
            {value for value in references if value is not None} - set(releases)
        )
        if missing:
            raise ContractError(
                "onboard release state references unknown releases: "
                + ", ".join(missing)
            )
        ...
        for release_id, release in releases.items():
            ...
            if (
                release["release_class"] == "field-development"
                and release["status"] != "field-development"
            ):
                raise ContractError(
                    "field release acquired a qualified status classification"
                )
    ...
    def _load_state(self) -> dict[str, Any]:
        ...
        return self._validate_state(
            _canonical_document(self.state_path, label="onboard release state")
        )
    ...
    def _load_status_index(self) -> dict[str, Any] | None:
        ...
        index = _canonical_document(
            self.status_path, label="onboard release-status index"
        )
        trust = (
        ...
        if incoming is not None:
            ...
            if current is not None:
                ...
                if incoming_sequence < current_sequence:
                    raise ContractError(
                        "stale release-status index cannot replace onboard safety state"
                    )
                if (
                    incoming_sequence == current_sequence
                    and incoming["index_id"] != current["index_id"]
                ):
                    raise ContractError(
                        "conflicting release-status index at current sequence"
                    )
                retained = current["statements"]
                if incoming["statements"][: len(retained)] != retained:
                    raise ContractError(
                        "release-status index does not extend the onboard signed chain"
                    )
    ...
    def _set_recovery_state(state: dict[str, Any]) -> None:
        ...
        for role in ("active_release_id", "qualified_anchor_release_id"):
            ...
            if (
                ...
                and state["releases"][release_id]["status"] == "unsafe"
            ):
    ...
    def _install_release(
        self, verified: VerifiedBundle, metadata: Mapping[str, Any]
    ) -> None:
        release_id = metadata["release_id"]
        destination = self.releases_root / release_id
        staging = Path(
            tempfile.mkdtemp(prefix=f".{release_id}.", dir=self.releases_root)
        )
        try:
            ...
            if (
                ...
                raise ContractError(
                    "bundle identity changed between verification and extraction"
                )
            payload = extracted / "payload"
            if payload.is_symlink() or not payload.is_dir():
                raise ContractError(
                    "verified bundle did not contain a release payload directory"
                )
            for child in sorted(
                payload.iterdir(), key=lambda value: value.name.encode("utf-8")
            ):
                os.replace(child, staging / child.name)
            meta = extracted / "META"
            os.replace(meta / "bundle-manifest.json", staging / "bundle-manifest.json")
            os.replace(
                meta / "release-manifest.json", staging / "release-manifest.json"
            )
    ...
    def _verify_release_tree(
        ...
        observed_receipt = (
            ...
            if receipt is not None
                ...
                root / "manifest.json", label="staged release receipt"
            )
        )
        ...
        bundle = _canonical_document(
            root / "bundle-manifest.json", label="staged bundle manifest"
        )
        release = _canonical_document(
            root / "release-manifest.json", label="staged release manifest"
        )
        self.registry.validate("bundle-manifest", bundle)
        validate_release_metadata(release, self.registry)
        if (
            _sha256(root / "bundle-manifest.json")
            != observed_receipt["bundle_manifest_sha256"]
            or _sha256(root / "release-manifest.json")
            != observed_receipt["release_manifest_sha256"]
            or bundle["release_manifest_sha256"]
            != observed_receipt["release_manifest_sha256"]
        ):
        ...
        if (
            ...
            raise ContractError(
                "staged release logical identity differs from its receipt"
            )
        ...
        for relative, item in expected.items():
            ...
            if observed[relative] != item["type"]:
                raise ContractError(
                    f"staged release type differs from signed index: {relative}"
                )
            expected_mode = (
                0o550 if item["type"] == "directory" or item["mode"] & 0o111 else 0o440
            )
            if stat.S_IMODE(path.stat().st_mode) != expected_mode:
                raise ContractError(
                    f"staged release mode differs from immutable policy: {relative}"
                )
            if item["type"] == "file" and (
                path.stat().st_size != item["size"] or _sha256(path) != item["sha256"]
            ):
                raise ContractError(
                    f"staged release content differs from signed index: {relative}"
                )
    ...
    def _existing_release(self, metadata: Mapping[str, Any]) -> bool:
        ...
        if not self._receipt_matches(receipt, metadata):
            raise ContractError(
                "existing staged release differs from the verified bundle identity"
            )
        return True
    ...
    def stage(
        ...
        *,
        ...
        if verified.bundle_manifest["component"] != "drone":
            raise ContractError(
                "onboard release staging accepts only the drone component"
            )
        release = verified.release_manifest
        with self._locked():
            state = self._load_state()
            ...
            if release["release_class"] == "qualified":
                if statement is None or statement["version"] != release.get("version"):
                    raise ContractError(
                        "qualified release lacks an exact verified status statement"
                    )
            ...
            if observed is not None:
                ...
                if any(observed[key] != value for key, value in immutable.items()):
                    raise ContractError(
                        "onboard release metadata changed for an immutable release ID"
                    )
            else:
    ...
    def _remove_release(self, release_id: str, state: dict[str, Any]) -> None:
        if release_id in self._protected_release_ids(state):
            raise ContractError(
                f"release {release_id} is protected from garbage collection"
            )
        destination = self.releases_root / release_id
        if destination.exists() or destination.is_symlink():
            ...
            if receipt.get("release_id") != release_id:
                raise ContractError(
                    "garbage collection release receipt identity mismatch"
                )
        ...
        *,
        ...
        with self._locked():
            ...
            if release["release_class"] == "qualified":
                if statement is None or statement["version"] != release["version"]:
                    raise ContractError(
                        "qualified activation lacks an exact verified status statement"
                    )
                status_statement_id = statement["statement_id"]
                if statement["status"] in {"withdrawn", "unsafe"}:
                    ...
                    if not allow_unsafe_recovery or viable:
                        raise ContractError(
                            "unsafe release is not eligible for last-resort recovery"
                        )
                    recovery_only = True
        ...
        with self._locked():
            ...
            if release["release_class"] == "qualified":
                if not explicit_qualified_action:
                    raise ContractError(
                        "qualified acceptance requires explicit qualified authority"
                    )
                if release["status"] == "unsafe" and not authorization.recovery_only:
                    raise ContractError(
                        "unsafe qualified release lacks recovery-only authorization"
                    )
                if release["status"] not in {"qualified", "unsafe"}:
                    raise ContractError(
                        "qualified release status no longer permits acceptance"
                    )
            elif explicit_qualified_action:
                raise ContractError(
                    "field-development release cannot use qualified authority"
                )
    ...
    def authorize_rollback(
        ...
        *,
        ...
        """Bind an explicit operator rollback to the retained previous release."""

        with self._locked():
            state = self._load_state()
            if state["rollback_release_id"] != release_id:
                raise ContractError(
                    "operator rollback target is not the retained previous release"
                )
            selected_index, latest = self._verified_status_update(status_index)
            self._apply_statuses(state, selected_index, latest)
            if selected_index is not None:
            ...
            if release["release_class"] == "qualified":
                if statement is None or statement["version"] != release["version"]:
                    raise ContractError(
                        "qualified rollback lacks an exact verified status statement"
                    )
                if statement["status"] in {"withdrawn", "unsafe"}:
                    raise ContractError(
                        f"{statement['status']} release cannot be selected for operator rollback"
                    )
                status_statement_id = statement["statement_id"]
            if selected_index is None:
                state = self._commit_state(state)
    ...
    def record_rollback_acceptance(
        self, authorization: ActivationAuthorization
    ) -> dict[str, Any]:
        """Durably accept an explicitly requested, newly health-checked rollback."""

        value = dict(authorization.__dict__)
        if value["authorization_id"] != _authorization_identity(value):
            raise ContractError("rollback authorization identity mismatch")
        with self._locked():
            state = self._load_state()
            if (
                state["state_id"] != authorization.state_id
                or state["generation"] != authorization.state_generation
                or state["rollback_release_id"] != authorization.release_id
                or state["status_index_id"] != authorization.status_index_id
            ):
                raise ContractError("operator rollback authorization is stale")
            target = state["releases"][authorization.release_id]
            if (
                target["release_class"] == "qualified"
                and target["status"] != "qualified"
            ):
                raise ContractError(
                    "qualified rollback release is no longer deployable"
                )
            previous = state["active_release_id"]
            if previous is None:
                raise ContractError(
                    "operator rollback has no active release to preserve"
                )
            state["active_release_id"] = authorization.release_id
            state["rollback_release_id"] = previous
            state["candidate_release_id"] = None
            ...
            target["accepted_sequence"] = max(accepted, default=0) + 1
            if target["release_class"] == "field-development":
                state["field_history"] = [
                    authorization.release_id,
                    *(
                        item
