...
def create_status_statement(
    *,
    ...
    value: dict[str, Any] = {
        "schema_version": "1",
        "statement_id": "0" * 64,
        "sequence": 1 if previous_global is None else int(previous_global["sequence"]) + 1,
        ...
        "previous_statement": None if previous_global is None else _reference(previous_global),
    ...
    validate_status_transition(
        previous_release, value, previous_global=previous_global
    )
    return value
...
def verify_status_statement(
    ...
    public = trusted_public_key(
        trusted_signers, statement["signer_id"], "release-status"
    )
    verify(
        public,
        statement["signature"],
...
def verify_status_index(
    ...
    public = trusted_public_key(
        trusted_signers, index["signer_id"], "release-status"
    )
    ...
    for statement in ordered:
        ...
        if version in versions and versions[version] != release_id:
            raise ContractError("release-status index maps a version to multiple releases")
    ...
    if previous_index is not None:
        ...
        for value in latest.values():
            if value["version"] == version and value["release_id"] != release_id:
                raise ContractError("release version is already bound to another release identity")
    observed_id = None if previous_release is None else str(previous_release["statement_id"])
